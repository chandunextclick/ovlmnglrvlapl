...
class EmployeesReportDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('name', function ($row) {
            return view('components.employeereport', [
                'user' => $row
            ]);
        });
        ...
        $datatables->editColumn('employee_id', function ($row) {
            return '<a href="' . route('employee-report.show', [$row->id]) . '" class="text-darkest-grey">' . $row->employee_id . '</a>';
        });
