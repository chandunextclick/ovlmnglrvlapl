...
class ProjectsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if (
                ...
                || ($this->editProjectsPermission == 'added' && (user()->id == $row->added_by || in_array(user()->id, $headIds)))
                ...
                if($row->priority == 0){
                    ...
                    $action .= '<a class="dropdown-item priority" href="javascript:;" data-user-id="' . $row->id . '" data-oper=1>
                        <i class="fa fa-edit mr-2"></i>
                        ' . trans('app.addpriority') . '
                    </a>';

                }else{

                    $action .= '<a class="dropdown-item priority" href="javascript:;" data-user-id="' . $row->id . '" data-oper=0>
                        <i class="fa fa-edit mr-2"></i>
                        ' . trans('app.removepriority') . '
                    </a>';
        ...
        // modified by chandu to add project category column on the datatable on 13/09/23 
        ...
        $datatables->editColumn('category_name', function ($row) {
            return '<a href="' . route('projects.show', [$row->id]) . '" class="text-darkest-grey">' . $row->category_name . '</a>';
        });
        ...
        $datatables->rawColumns(['project_name', 'action', 'completion_percent', 'members', 'status', 'client_id', 'check','project_short_code','category_name']);
    ...
    public function query(Project $model)
    {
        ...
        $model = $model
            ...
            ->leftJoin('project_category', 'project_category.id', 'projects.category_id') // modified by chandu to add project category column on the query on 13/09/23 
            ->leftJoin('users', 'project_members.user_id', 'users.id')
            ->leftJoin('project_departments', 'project_departments.project_id', 'projects.id')
            ->leftJoin('teams', 'project_departments.team_id', 'teams.id')
            ->leftJoin('users as client', 'projects.client_id', 'users.id')
            ->selectRaw('projects.id, projects.project_short_code,projects.category_id,projects.hash, projects.added_by, projects.project_name, projects.start_date, projects.deadline, projects.client_id,
              projects.completion_percent, projects.project_budget, projects.currency_id,project_category.category_name,
            projects.status, users.name,teams.head_id, client.name as client_name, client.email as client_email, projects.public,projects.priority,
        ...
        if (!is_null($request->team_id) && $request->team_id != 'all') {
            $model->where('teams.id', $request->team_id);
        }
        ...
        return $this->setBuilder('projects-table', 2)
            ->parameters([
                ...
                'createdRow' => 'function (row, data, dataIndex) {
                    if (data.priority == 1) {
                        $(row).addClass("bg-warning"); // Add the "row-red" class for rows with priority 1
                    }
                }',
            ])
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.projects.projectCategory') => ['data' => 'category_name', 'name' => 'category_name', 'title' => __('modules.projects.projectCategory')], // modified by chandu to add project category column on the datatable on 13/09/23 
            ...
            __('app.start_date') => ['data' => 'start_date', 'name' => 'start_date', 'title' => __('Startdate')], // modified by chandu to add project startdate column on the datatable on 19/09/23 
