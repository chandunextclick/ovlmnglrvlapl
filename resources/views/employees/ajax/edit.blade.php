            ...
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-lg-4 col-md-6">
                                ...
                                :fieldValue="$employee->employeeDetail->guardian"   fieldName="guardianname" fieldRequired="true" :fieldPlaceholder="__('placeholders.name')">
                                </x-forms.text>
                    </div>
                    <div class="col-lg-4 col-md-6">
                                <x-forms.text fieldId="aadhar" :fieldLabel="__('modules.employees.aadhar')"
                                :fieldValue="$employee->employeeDetail->aadhar"    fieldName="aadhar" fieldRequired="true" :fieldPlaceholder="__('placeholders.aadhar')" >
                                </x-forms.text>
                    </div>
                    <div class="col-lg-4 col-md-6">
                                <x-forms.text fieldId="accno" :fieldLabel="__('modules.employees.accountNo')"
                                :fieldValue="$employee->employeeDetail->accountno"    fieldName="accno" fieldRequired="true" :fieldPlaceholder="__('placeholders.accountno')">
                                </x-forms.text>
                    </div>
                    <div class="col-lg-4 col-md-6">
                    ...
                    <div class="col-lg-4 col-md-6">
                                ...
                                :fieldValue="$employee->employeeDetail->esino"    fieldName="esino" fieldRequired="false" :fieldPlaceholder="__('placeholders.esino')">
                                </x-forms.text>
                    </div>
                </div>
                <h4 class="mb-0 p-20 f-21 font-weight-normal text-capitalize border-bottom-grey">
...
<script>
    $(document).ready(function() {
        ...
        @if ($employeememberdetail)

            @if(count($employeememberdetail)==0)

            $('.member-row[data-row-id="1"]').removeClass('d-none')

            @endif

        @foreach ($employeememberdetail as $detail)
            ...
            newRow.find('[id^="member"]').attr('id', 'member' + rowCount).attr('name', 'member[' + rowCount + ']').attr('value','{{$detail->name}}');
            newRow.find('[id^="mbrelation"]').attr('id', 'mbrelation' + rowCount).attr('name', 'mbrelation[' + rowCount + ']').attr('value','{{$detail->relation}}');
            newRow.find('[id^="date_of_birth"]').attr('id', 'date_of_birth' + rowCount).attr('name', 'mbdate_of_birth[' + rowCount + ']').attr('value','{{$detail->dob}}');
            newRow.find('[id^="mbmobile"]').attr('id', 'mbmobile' + rowCount).attr('name', 'mbmobile[' + rowCount + ']').attr('value','{{$detail->mobile}}');

            // Append the new row to the memberDetails div
            $('#memberDetails').append(newRow);

            // Initialize datepicker for the new date_of_birth field
        ...
        $('#save-form').click(function() {
            ...
            $('.member-row[data-row-id="1"]').remove();
