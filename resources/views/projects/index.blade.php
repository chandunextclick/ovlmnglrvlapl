...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('app.priority')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" name="priority" id="priority" data-container="body"
                            data-size="8">
                            <option value="all">@lang('app.all')</option>
    ...
    <div class="content-wrapper">
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar">
            ...
            <div class="btn-group ml-0 ml-lg-3 ml-md-3" role="group">
                ...
                <a href="javascript:;" class="btn btn-secondary f-14 show-priority" data-toggle="tooltip"
                    data-original-title="@lang('app.priority')"><i class="side-icon bi bi-kanban"></i></a>
...
@push('scripts')
    ...
    <script>
        ...
        const showTable = () => {
            ...
            // console.log(window.LaravelDataTables["projects-table"]);
            window.LaravelDataTables["projects-table"].draw(false);
        }

        $('#client_id, #status, #employee_id, #team_id, #category_id, #pinned, #priority, #progress').on('change keyup',
            function() {
        ...
        $('body').on('click', '.priority', function() {
            ...
            Swal.fire({
                title: "@lang('messages.sweetAlertTitle')",
                text: "@lang('messages.priorityMessage')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('messages.confirmPriority')",
                cancelButtonText: "@lang('app.cancel')",
                customClass: {
                    confirmButton: 'btn btn-primary mr-3',
                    cancelButton: 'btn btn-secondary'
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('projects.change_priority', ['id' => ':id', 'oper' => ':oper' ]) }}";
                    url = url.replace(':id', id).replace(':oper', oper);
