            ...
            <div class="select-status">
                <select class="form-control select-picker" name="status" id="status" data-live-search="true" data-size="8">
                    ...
                    @foreach ($taskBoardStatus as $status)
                        <option value="{{ $status->id }}" {{ (request('completed') == 'yes' && $status->slug == 'completed') ? 'selected' : '' }}>{{ $status->slug == 'completed' || $status->slug == 'incomplete' ? __('app.' . $status->slug) : $status->column_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
                    ...
                    <div class="select-others">
                        <select class="form-control select-picker" id="assignedTo" data-live-search="true"
                            ...
                            @foreach ($employees as $employee)
                                @if(request('empreportid'))

                                <x-user-option :user="$employee"
                                               :selected="request('assignee') == 'me' && $employee->id == request('empreportid')">
                                </x-user-option>
