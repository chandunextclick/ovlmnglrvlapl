...
<div class="add-client bg-white rounded">
        <form method="POST" action="{{ route('rankings.rankincountryupdate') }}">
            @csrf
                <h4 class="mb-0 p-20 f-21 font-weight-normal text-capitalize border-bottom-grey">
                    Edit Ranking Country</h4>
                <div class="row p-20">
                    <div class="col-md-4">
                        <div class="form-group">
                        <label class="f-14 text-dark-grey mb-12" data-label="true" for="country_name">Country Name
                        <sup class="f-14 mr-1">*</sup>
                        </label>
                        <input type="text" class="form-control height-35 f-14" placeholder="Country Name" value="<?=$country->ranking_country?>" name="country_name" id="country_name" autocomplete="off" required>
                        <input type="hidden" class="form-control height-35 f-14" placeholder="Country Name" value="<?=$country->id?>" name="country_id" id="country_id" autocomplete="off" required>
                        </div>
                    
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                        <label class="f-14 text-dark-grey mb-12" data-label="true" for="increase_percent">Increase Percent
                        <sup class="f-14 mr-1">*</sup>
                        </label>
                        <input type="text" class="form-control height-35 f-14" placeholder="Increase Percent" value="<?=$country->increase_percent?>" name="increase_percent" id="increase_percent" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                        ...
                        <select class="form-control height-35 f-14" placeholder="client"  name="client" id="client"  required>                 
                            <option <?= ($country->client == "EDOXI")? 'selected':'' ?> value="EDOXI">EDOXI</option>
                            <option <?= ($country->client == "TIMEMASTER")? 'selected':'' ?> value="TIMEMASTER">TIME MASTER</option>
                            <option <?= ($country->client == "TIMETRAINING")? 'selected':'' ?> value="TIMETRAINING">TIME TRAINING</option>                
                        </select>  
                    </div>
