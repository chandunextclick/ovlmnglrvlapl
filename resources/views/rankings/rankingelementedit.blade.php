...
<div class="add-client bg-white rounded">
        <form method="POST" action="{{ route('rankings.rankinelementupdate') }}">
            @csrf
                <h4 class="mb-0 p-20 f-21 font-weight-normal text-capitalize border-bottom-grey">
                    Edit Ranking Element</h4>
                <div class="row p-20">
                    <div class="col-md-4">
                        <div class="form-group">
                        <label class="f-14 text-dark-grey mb-12" data-label="true" for="course_name">Element Name
                        <sup class="f-14 mr-1">*</sup>
                        </label>
                        <input type="text" class="form-control height-35 f-14" placeholder="Element Name" value="<?=$element->ranking_element?>" name="element_name" id="element_name" autocomplete="off" required>
                        <input type="hidden" class="form-control height-35 f-14" placeholder="Element Name" value="<?=$element->id?>" name="element_id" id="element_id" autocomplete="off" required>
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                        <label class="f-14 text-dark-grey mb-12" data-label="true" for="element_name">Increase Percent
                        <sup class="f-14 mr-1">*</sup>
                        </label>
                        <input type="text" class="form-control height-35 f-14" placeholder="Increase Percent" value="<?=$element->increase_percent?>" name="increase_percent" id="increase_percent" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                        ...
                        <select class="form-control height-35 f-14" placeholder="client"  name="client" id="client"  required>                 
                            <option <?= ($element->client == "EDOXI")? 'selected':'' ?> value="EDOXI">EDOXI</option>
                            <option <?= ($element->client == "TIMEMASTER")? 'selected':'' ?> value="TIMEMASTER">TIME MASTER</option>
                            <option <?= ($element->client == "TIMETRAINING")? 'selected':'' ?> value="TIMETRAINING">TIME TRAINING</option>                
                        </select>  
                    </div>
